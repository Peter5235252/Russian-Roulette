...
async function queryAiProvider(
  ...
  const diff = (difficulty || 'NORMAL').toUpperCase();

  // Scale reasoning effort, max tokens, temperature, and thinking depth according to difficulty
  ...
  if (p === "chatgpt" || p === "openai") {
    ...
    const reqBody: any = {
      ...
      messages: [
        { role: "system", content: systemInstruction + ` [Difficulty Level: ${diff}] [Reasoning Depth: ${reasoningEffort.toUpperCase()}]. Conduct step-by-step probability calculation in 'reasoning'. Output strictly valid JSON with keys: action ('USE_ITEM'|'SHOOT'), itemIndex (integer, -1 if SHOOT), target ('player'|'dealer'), reasoning (string).` },
  ...
  if (p === "grok" || p === "xai") {
    ...
    const response = await fetch("https://api.x.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: systemInstruction + ` [Difficulty: ${diff}] [Reasoning Effort: ${reasoningEffort.toUpperCase()}]. Perform thorough analytical calculation in 'reasoning' before deciding. Output strictly valid JSON with keys: action ('USE_ITEM'|'SHOOT'), itemIndex (integer, -1 if SHOOT), target ('player'|'dealer'), reasoning (string).` },
          { role: "user", content: prompt }
        ]
      })
    });
  ...
  if (p === "mistral") {
    ...
    const response = await fetch("https://api.mistral.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: maxTokens,
        messages: [
          { role: "system", content: systemInstruction + ` [Difficulty: ${diff}] [Reasoning Depth: ${reasoningEffort.toUpperCase()}]. Conduct multi-step game tree calculation in your 'reasoning' field before picking action. Output strictly valid JSON with keys: action ('USE_ITEM'|'SHOOT'), itemIndex (integer, -1 if SHOOT), target ('player'|'dealer'), reasoning (string).` },
          { role: "user", content: prompt }
        ]
      })
    });
  ...
  if (p === "claude" || p === "anthropic") {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: maxTokens,
        temperature,
        system: systemInstruction + ` [Difficulty: ${diff}] [Reasoning Effort: ${reasoningEffort.toUpperCase()}]. Perform step-by-step game tree evaluation in your 'reasoning' property. You MUST output strictly valid JSON only containing keys: action ('USE_ITEM'|'SHOOT'), itemIndex (integer, -1 if SHOOT), target ('player'|'dealer'), reasoning (string). Do not include markdown codeblocks or text outside JSON.`,
        messages: [
          { role: "user", content: prompt }
        ]
      })
...
`;
    ...
    const parsed = await queryAiProvider(provider, apiKey, customModel, difficultyPersona, prompt, difficulty);
